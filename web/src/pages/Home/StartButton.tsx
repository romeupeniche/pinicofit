...
const StartButton: React.FC<StartButtonProps> = ({ text, ...props }) => {
  return (
    <button
      className="border border-white text-left mt-4 px-4 w-60 rounded-2xl h-14 relative text-black text-2xl font-semibold group cursor-pointer"
      type="button"
      {...props}
    >
      <p className="group-hover:opacity-0 duration-300 font-bold">{text}</p>
      <div className="bg-brand-accent/60 rounded-xl h-12 w-1/4 flex items-center justify-center absolute right-1 top-1 group-hover:w-58 z-10 duration-500">
        <svg
          width="25px"
          height="25px"
          viewBox="0 0 15 15"
